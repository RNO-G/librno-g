...
#include <zlib.h> 
#include "../src/rno-g.h" 
//#include "../src/rno-g-baseline-shift.h" 
#include "FFTtools.h" 
...
/*
static bool enable_baseline_shift = false; 
static rno_g_baseline_shift_strategy_t baseline_strat; 
static const void * baseline_p; 
void enable_quick_plot_baseline_shift(rno_g_baseline_shift_strategy_t strat = RNO_G_BASELINE_SHIFT_WINDOW_MEAN, const void * p = 0)
{
  baseline_strat = strat; 
  baseline_p = p; 
  enable_baseline_shift = true; 
}
*/

//void disable_quick_plot_baseline_shift() 
//{
//  enable_baseline_shift = false; 
//
//}
//
void quick_plot(const char * file, int ev = 0, int symmetric=1, int Nev = 1, int save = false, int resfactor=1,int mask=16777215, int min_rms_sample = 0, int max_rms_sample = 400, int zero_sub=0, int median_sub = 0)
{
  ...
  for (int iev = 0; iev < Nev; iev++) 
  {
    ...
    /*
    if (enable_baseline_shift) 
    {
      int16_t shifts[RNO_G_MAX_RADIANT_WINDOWS_PER_WF]; 
      rno_g_baseline_shift_compute(&wf, baseline_strat, baseline_p, RNO_G_ALL_RADIANTS_MASK); 
      int nwind = wf.radiant_nsamples >> 7; 
      for (int i = 0; i < RNO_G_NUM_RADIANT_CHANNELS; i++) 
      {
        rno_g_baseline_shift_apply(nwind, wf.radiant_waveforms[i], wf.radiant_waveform_baseline_shifts[i],0); 
        printf("BASELINE SHIFTS FOR CHANNEL %d: ",i); 
        for (int j = 0; j < nwind; j++) 
        {
          printf(" %hd ", wf.radiant_waveform_baseline_shifts[i][j]); 
        }
        printf("\n"); 
      }
    }
