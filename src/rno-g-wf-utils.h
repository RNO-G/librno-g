...
/** Returns a scramble score for the waveform.
 * This is \sum ( difference across seams without  unscrambling applied)^2  - \sum (difference across seams with unscrambling applied)^2
 * This will tend to be positive if not scrambled, negative if scrambled
 */
int rno_g_scramble_score(unsigned nsamp, const int16_t * wf);
void  rno_g_unscramble(unsigned nsamp, int16_t *wf);



/** Difference histogram between nsamples. What  is histogrammed is either
* wf[i] - wf[i-delta_nsamples] or the absolute value of this.
**/
