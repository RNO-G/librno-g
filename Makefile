...
#get the git hash. This doesn't work on macos for some reason so just hardcode mac os... 
OS=$(shell uname) 
ifeq ($(OS),Darwin)
$(shell  if [ ! -f src/rno-g-version.c ] ; then cp src/rno-g-version.dummy.c src/rno-g-version.c ; fi )
else 
